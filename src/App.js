  ...
  render() {
    return (
      <div className="container">
        ...
        <hr/>
        <h4>Todo Count: <span className="badge badge-secondary badge-pill">{this.state.todos.length}</span></h4>
        <ul className="list-group">
          {this.state.todos.map((todo, index) =>
          <li className="list-group-item" key={index}>
            <h4 className="list-group-item-heading">{todo.todoTitle}<small><span className="badge badge-pill badge-info">{todo.todoPriority}</span></small></h4>
            
            <p><span><i class="fa fa-user" aria-hidden="true"></i></span>{todo.todoResponsible}</p>

            <p>{todo.todoDescription}</p>

            <button className="btn btn-danger btn-sm" onClick={this.handleRemoveTodo.bind(this, index)}><span><i class="fa fa-trash-o" aria-hidden="true"></i></span>Delete</button>
          </li>
          )}
          </ul>
      </div>
